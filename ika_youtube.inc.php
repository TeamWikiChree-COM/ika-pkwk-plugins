...
if (!defined('PLUGIN_IKA_YOUTUBE_WIDTH'))  define('PLUGIN_IKA_YOUTUBE_WIDTH',  640); // 動画幅の既定値（px）
if (!defined('PLUGIN_IKA_YOUTUBE_ASPECT')) define('PLUGIN_IKA_YOUTUBE_ASPECT', (9.0 / 16.0)); // 動画の縦横比


function plugin_ika_youtube_convert() {
	list($id, $width, $loop, $start, $autoplay) = func_get_args();

	$id = trim($id);
	if ($id) {
		$id = urlencode($id);
		$width = ($width > 0)? (int)$width : PLUGIN_IKA_YOUTUBE_WIDTH;
		$height = round($width * PLUGIN_IKA_YOUTUBE_ASPECT);
		...
		if ($widgetTag && !$included) {
			$aspect = sprintf('%.5f', 100 * PLUGIN_IKA_YOUTUBE_ASPECT);
			$widgetTag .= <<<EOT
<style>
...
EOT;
	...
	return ($widgetTag)? $widgetTag : '#ika_youtube(id[,[width][,[loop][,[start][,autoplay]]]])';
}
